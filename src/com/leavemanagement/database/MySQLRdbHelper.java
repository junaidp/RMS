...
public class MySQLRdbHelper {
	...
	public String fetchAllReport(HashMap<String, Integer> reportData, String rootDir) {
		...
		try {
			...
			FileOutputStream fileOut = new FileOutputStream(rootDir + "/FullReport/reportnew.xlsx");
	...
	private void allJobReport(ArrayList<AllJobsReportDTO> jobReports, XSSFWorkbook workbook, Integer userId,
			Integer jobId, String rootDir) throws DocumentException, IOException {
		XSSFSheet worksheet = workbook.createSheet("All Jobs Report");
		XSSFRow rowUserName = worksheet.createRow((short) 0);
		XSSFRow rowHeading = worksheet.createRow((short) 1);
		...
		for (int i = 0; i < jobReports.size(); i++) {
			...
			for (TimeSheet timeSheet : jobReports.get(i).getListTimeSheet()) {
				...
				if (timeSheet.getHours() > 0) {
					count++;
					rowNum = rowNum + 1;
					XSSFRow row = worksheet.createRow((short) rowNum == 1 ? 2 : rowNum == 2 ? 3 : rowNum);
	...
	private void allJobReportPDF(ArrayList<AllJobsReportDTO> jobReports, XSSFWorkbook workbook, Integer userId,
			Integer jobId, String rootDir) throws DocumentException, IOException {
	...
	public String fetchJobWiseReport(HashMap<String, Integer> reportData, String rootDir) {
		...
		try {
			...
			FileOutputStream fileOut = new FileOutputStream(rootDir + "/JobWiseReport/reportnew.xlsx");// "D:\\POI111.xls"
			XSSFWorkbook workbook = new XSSFWorkbook();
			XSSFSheet worksheet = workbook.createSheet("Job wise Report");
			XSSFRow rowJobName = worksheet.createRow((short) 0);
			XSSFRow rowHeading = worksheet.createRow((short) 2);
			XSSFRow rowHeading2 = worksheet.createRow((short) 3);
